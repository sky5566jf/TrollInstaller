...
| 启动成功提示 | 服务真正就绪后，弹出本地通知「M巨魔助手 启动成功」（由系统派发，App 退出后横幅仍显示） |
...
App 被打开（或 NEHotspotHelper 冷启动唤醒）后，会轮询本地 HTTP 服务 `http://127.0.0.1:8588/status`，**当确认服务真正监听并响应 200** 时，通过 `UNUserNotificationCenter` 派发一条本地通知：

- **标题**：`M巨魔助手`
- **正文**：`启动成功，服务已就绪`
- 通知由**系统**派发，App 随后主动退出（bootstrap-only），横幅不依赖 App 进程存活，退出后仍正常显示
...
| 超时处理 | 10 秒内未就绪则安静退出，**不弹通知**（服务可能本就仅靠 supervisor 运行） |
| 通知 identifier | `com.matisu.trollassistant.startup`（同 App 内唯一，避免自我覆盖） |
| 权限 | 首次运行向用户申请一次本地通知权限（仅用于横幅，无需远程推送） |

> **跨 App 不冲突**：iOS 本地通知按 bundle ID 沙盒隔离，每个 App 有独立的通知中心。即使其他 App 同时弹出内容不同的提示，也会作为独立横幅分别展示，互不影响。通知 identifier 仅在同一个 App 内部用于去重/替换，跨 App 不存在冲突。
