...
| 启动成功提示 | 手动打开 App 后，轮询确认 8588 服务真正就绪，即弹本地通知「巨魔助手启动成功」（由系统派发，App 退出后横幅仍显示） |
...
手动打开 App 后，App 会先拉起 supervisor，再**轮询本地 HTTP 服务 `http://127.0.0.1:8588/status`**，当确认服务真正监听并响应 `200` 时，通过 `UNUserNotificationCenter` 派发一条本地通知：
...
> **跨 App 不冲突**：iOS 本地通知按 bundle ID 沙盒隔离，每个 App 有独立的通知中心。即使其他 App 同时弹出内容不同的提示，也会作为独立横幅分别展示，互不影响。
